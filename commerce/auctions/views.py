...
from .models import User, Watched, AuctionsListings,BidRecord,Comments
...
def index(request):
    ...
    if request.user.is_authenticated:
        watch = Watched.objects.all().filter(uid=request.user).count()
    else:
        watch = False
...
def listings(request, auction_id):
    ...
    added = Watched.objects.filter(uid=request.user, aid=auction_id)
...
def add_watchlist(request, auction_id):
    ...
    added = Watched.objects.filter(uid = request.user, aid = listing)
    if added:
        added.delete()
        return HttpResponseRedirect(reverse("listings", args=(auction_id,)))
    else:
        add = Watched(uid = request.user, aid = listing)
        add.save()
...
def watchList(request):
    watch = Watched.objects.all().filter(uid = request.user).values()
    items = []
...
def winnings(request):
    win = BidRecord.objects.filter(uid = request.user)
    if (win is not None):
